...
:: Processor performance time check interval - You may set to the max (5000) if your system fits the situation described on https://github.com/amitxv/PC-Tuning/blob/main/docs/post-install.md#configure-power-options
...
:: All the tweaks above should be very close to same result as having the Processor idle disabled, except not having cpu at 100% all the time.
...
:: Disable Energy Estimation logging / telemetry
...
:: Disable Fair Share CPU Scheduling
...
:: Disable HPET
REG ADD "HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Enum\HARDWARE\DEVICEMAP\ACPI\HPET" /v Enable /t REG_DWORD /d 0 /f
