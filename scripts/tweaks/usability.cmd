...
REG ADD "HKEY_CURRENT_USER\Software\Microsoft\Windows\CurrentVersion\Policies\Explorer" /v NoAutoUpdate /t REG_DWORD /d 1 /f
...
:: Someone mentioned that one should not disable MPO and Xbox Components, as it destroys FSE and games do not work on legacy flip. Here is how you enable it. By default it will be kept disabled.
:: REG DELETE "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\Dwm" /v OverlayTestMode /f
:: As for Xbox Components, you should look into the Gamebar, GameDVR and GameConfigStore tweaks above.
:: Maybe not all is needed to be changed as to have both FSE and legacy flip working. There is also some xbox services that I am not sure if related or not.
...
:: Ignore remote keyboard layout change
